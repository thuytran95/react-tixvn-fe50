...
export default function MovieDetailMainContentShowInfo(props) {
  ...
  const ngayChieu = ngayChieuGioChieu?.filter((value, index, ngayChieuGioChieu) => ngayChieuGioChieu.indexOf(value) === index);
 ...
 // lịchpim
  ...
  const thongTinRap = lichChieu?.filter(item =>   (format("MM/dd/yyyy", new Date(item.ngayChieuGioChieu)) === ngayChieu[activeNgay] ) );
  ...
  const thongTinRapCoPhim =  listRap?.filter(function(obj) { return rapCoPhimActive.indexOf(obj.maCumRap) !== -1; });
  const maCumRapThongTinPhim = thongTinRapCoPhim?.map(item => item.maCumRap)
  const thongTinRapDuocChon = thongTinRapActive?.filter(function(obj) { return maCumRapThongTinPhim?.indexOf(obj.maCumRap) !== -1; });
  const listMaRapDuocChon =thongTinRapDuocChon?.map(item=>item.maRap)
  const listMaRapDuocChonKhongLap = listMaRapDuocChon?.filter((value, index, listMaRapDuocChon) => listMaRapDuocChon?.indexOf(value) === index);
  ...
  const renderNgayChieu = () => {
    return ngayChieu?.map((ngay,index) => (
      <div className={activeNgayColor(index) || "show__info__movies__time__item"} key={index}  onClick={()=>{setActiveNgayColor(index,ngay)}}>
  ...
  const renderCumRapPhim =()=>{
    ...
    <div className={activeRapColor(index) || "row show__info__movies__pcinema__name "} key={index}  onClick={()=>{setActiveRapColor(index,item.maHeThongRap)}}>
  ...
  const renderRapPhim = ()=>{
    return thongTinRapCoPhim?.map((item,index) =>( 
      <MovieDetailMainContentShowInfoItem item={item} time={thongTinRap} ngayChieu={ngayChieu[activeNgay]} maRap={listMaRapDuocChonKhongLap} key={index}/>
    ))
  }
